...
class ChatActivity : ComponentActivity(){
    ...
    private fun sendNotification(message: Message,currSendToken:String) {
        Log.i("message",message.UID + message.message_content + message.sender_name)
        ...
        Log.i("beforeSend","firestore is iniitilaizewd")
        firestore.collection("Accounts")
            .get()
            .addOnSuccessListener { querySnapshot ->
                for (document in querySnapshot) {
                    Log.i("beforeSend","inside sendNotification onsuccess inside for")
                    var fcmToken = ""
                    if(document.id==curUid){
                     fcmToken = document.data["FCMTOKEN"] as String
                    }
                    Log.i("beforeSend", "inside sendNotification $fcmToken")
                    // Skip the sender's own token
                    if (document.id != curUid) {
                        Log.i("beforeSend","inside sendNotification documnt.id!=curreuid")
                        ...
                        notificationData.put("body", displayName?.plus(":")?.plus(message.message_content.toString()))
                        ...
                        Log.i("beforeSend","inside sendNotification call api before")
                        // Call the API to send the notification

                        if(currSendToken!=fcmToken) {
                            callApi(notification)
                        }
                        Log.i("beforeSend","inside sendNotification callAPi After")
                    }
                }
            }
            .addOnFailureListener { e ->
...
// Generate an access token for FCM
        var token :String= "AIzaSyDuWuFbBgy8a-GTLEJ5pjKsRfFzxLJdHiI"
        ...
        val request = Request.Builder()
            .url(url)
            .post(body)
            .header(name = "Authorization","Bearer AAAAgiYHW5Q:APA91bFjMIPo0ZeseYuJHGVDpSSMMDbFThvIgxHSfrXt4vG5EqWRnqFmTX6p81qSAAuCm9LEQpirv0ET2AaJtHnKmU2rrxXzYjoaXJ-U8WiNKE62DBY1W1Jqw5a_8QgK227DuYzD9CPC")
            .build()
